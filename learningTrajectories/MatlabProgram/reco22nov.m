...
visualisation2(y_trial_Tot{trial},sum(nbDof), totalTimeTrial(trial),reco{1}, ':b', realAlpha, nameFig);
...
nameFig = visualisation(PSI_z*mu_w{i}, sum(nbDof), z, i, 'r', nameFig);
nameFig = visualisation(PSI_z*(mu_w{i} + 1.96*sqrt(diag(sigma_w{i}))), sum(nbDof), z, i, '-.r', nameFig);
nameFig = visualisation(PSI_z*(mu_w{i}- 1.96*sqrt(diag(sigma_w{i}))), sum(nbDof), z, i, '-.r', nameFig);
if(i~=1)
    nameFig = visualisation(PSI_z*mu_w{1}, sum(nbDof), z, i, 'k', nameFig);
end
if(i ~=2)
    nameFig = visualisation(PSI_z*mu_w{2}, sum(nbDof), z, i, 'k', nameFig);
end
if(i ~=3)
    nameFig = visualisation(PSI_z*mu_w{3}, sum(nbDof), z, i, 'k', nameFig);
end
