...
for k=1:nbKindOfTraj
	i = size(data{k},2);
	% totalTimeTrial2(k)= floor(size(data{k}{i-1},1) /z)*z; %size(data{k}{i},1); %if we continue the movement to the end
	totalTimeTrial(k) = size(data{k}{i},1); %size(data{k}{i},1); %if we continue the movement to the end
	y_trial_Tot{k} = [];
    y_trial{k} = [];
	% y_trial_Tot2{k} = [];
	for j=1:nbDofTot
	    y_trial_Tot{k} =  [ y_trial_Tot{k} ; data{k}{i}(1: totalTimeTrial(k),j)  ] ; %data{k}{i}(:,2); data{k}{i}(:,3); data{k}{i}(:,4) ; data{k}{i}(:,5); data{k}{i}(:,6)];
		y_trial{k} = [ y_trial{k} ; data{k}{i}(1:nbData,j)];% Br{i}(1:nbData,1)*0.01 ; Br{i}(1:nbData,2)*0.02; Br{i}(1:nbData,3)*0.03]; 
        %y_trial_Tot2{k} =  [ y_trial_Tot2{k} ; data{k}{i-1}(1: totalTimeTrial2(k),j)  ] ; %data{k}{i}(:,2); data{k}{i}(:,3); data{k}{i}(:,4) ; data{k}{i}(:,5); data{k}{i}(:,6)];
    end
end
