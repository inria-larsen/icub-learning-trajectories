...
for step=1: floor(totalTimeTrial(1) /10): min(totalTimeTrial(1),z)
    ...
    for i=1:nbKindOfTraj
        ...
        PSI_reco{step}{i} = computeBasisFunction(z,nbFunctions(typeReco), nbDof(typeReco), alphaTest, floor(z/alphaTest), center_gaussian(typeReco), h(typeReco),step);
        %matrix of basis functions for all data that correspond to the first
        %nbData
        PSI_tot{step}{i} = computeBasisFunction(z,nbFunctions, nbDof, alphaTest, floor(z/alphaTest), center_gaussian, h,step);
     ...
     %   sigma{step}{i} = PSI_reco{step}{i}*sigma_w_reco{i}*PSI_reco{step}{i}' + accuracy*eye(size(PSI_reco{step}{i}*sigma_w_reco{i}*PSI_reco{step}{i}'));
        ...
        wtest = (PSI_reco{step}{i}'*PSI_reco{step}{i}+ 1e-6*eye(size( PSI_reco{step}{i}'*PSI_reco{step}{i},1))) \ PSI_reco{step}{i}'* y_trial_part{step};    
