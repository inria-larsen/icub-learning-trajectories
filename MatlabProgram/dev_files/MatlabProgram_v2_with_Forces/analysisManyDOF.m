...
for trial=1:nbTrial
    ...
    figure(13+trial);
    plot(wtrial{trial},'b');hold on;
        
    for(i=1:size(newmu{trial},2)) % for each step (more known data at each step)
        plot(newmu{trial}{i}, 'r');hold on;
        logTest(trial, i) = logLikelihood(wtrial{trial}, newmu{trial}{i}, newSigma{trial}{i})
    end
    figure(2)
    subplot(2,2,1);
    fig(size(fig,2) + 1) = plot([1:totalTimeTrial(trial) /size(logTest(trial,:),2):totalTimeTrial(trial)], logTest(trial,:), '+'); hold on;    

    fig(size(fig,2) + 1) = plot([1:totalTimeTrial(trial) /size(logTest(trial,:),2):totalTimeTrial(trial)], logTest(trial,:)); hold on;    
end
 %plot([1:ceil(totalTimeTrial(trial) /size(logTest(trial,:),2)):totalTimeTrial(trial)],mean(logTest));
i=1
...
subplot(2,2,2);
type =1;
courb = PSI_inf{trial}{totstep}*mu_w{i};
...
for j=1:nbTrial
    fig(size(fig,2) + 1) =plot([1:floor(totalTimeTrial(trial) /nbStep):totalTimeTrial(nbStep)  ], stock_tps(j,:), ':', 'Color', [0,(nbStep -j)/nbStep,j/nbStep ]);hold on;
end
