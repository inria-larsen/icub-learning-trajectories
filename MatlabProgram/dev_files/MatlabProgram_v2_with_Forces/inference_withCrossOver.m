...
clearvars -except totTest stock_tps PSI_mean
 load('Data/dataDistributionManyDOF.mat');
...
for step=1: floor(totalTimeTrial(trial) /10):z / mu_alpha(i) %totalTimeTrial(1)
    tstart = tic;
...
%     logkike(step) = logLikelihood( y_trial_Tot{trial}', u', S); 
    ...
    stock_tps(totTest,:) = telapsed;
    PSI_inf = PSI_mean;
      alpha_inf = mu_alpha

    trajInf = (PSI_inf*newmu{i});
    minTime = min(size(trajInf,1), size(y_trial_Tot{trial},1))/nbDofTot;
    trajErr(1,:) = abs(trajInf(1:minTime) - y_trial_Tot{trial}(1:minTime));
    stock_err(totTest,:) = trajErr;
 end
 %   Drawing;
