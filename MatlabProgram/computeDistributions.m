...
%    % trajectory i
%    nf = figure;         
%    nf = visualisation(PSI_z*mu_w{i}, 1, z, 'r', nf, size(nf,2));
%    nf = visualisation(PSI_z*(mu_w{i} + 1.96*sqrt(diag(sigma_w{i}))), 1, z, '-.r', nf, size(nf,2));
%    nf = visualisation(PSI_z*(mu_w{i}- 1.96*sqrt(diag(sigma_w{i}))), 1, z, '-.r', nf, size(nf,2));
%    for j=1:var(i)
%        nf = visualisation2(y{i}{j}(1:totalTime(i,j),1), 1, totalTime(i,j), 'b', alpha{i}(j) , nf, size(nf,2));hold on;
%    end 
%     
%     if(i==1)
