...
%in this 22 nov version we take the real force information

fahead = fopen('Data/firstPoint.txt', 'r');
ftop = fopen('Data/secondPoint.txt', 'r');
fRight = fopen('Data/thirdPoint.txt', 'r');
...
for i=1:var(1)
    ...
    y{1}{i} = [ Br{i}(:,1) ; Br{i}(:,2); Br{i}(:,3); Br{i}(:,4) ; Br{i}(:,5); Br{i}(:,6)];
...
y_trial_Tot{1} =  [ Br{i}(:,1) ; Br{i}(:,2); Br{i}(:,3); Br{i}(:,4); Br{i}(:,5)*0.02; Br{i}(:,6)]; % the desired trajectory
...
y_trial_Tot{2} =  [ Ba{i}(:,1) ; Ba{i}(:,2); Ba{i}(:,3); Ba{i}(:,4) ; Ba{i}(:,5); Ba{i}(:,6)];
...
y_trial_Tot{3} =  [ Bt{i}(:,1) ; Bt{i}(:,2); Bt{i}(:,3); Bt{i}(:,4); Bt{i}(:,5); Bt{i}(:,6)];
